...
if [ $SLURM_ARRAY_TASK_ID -eq 0 ];then
    srun python 3_code/tf_vae_tfrecords.py -c triton --data_path /tmp/$SLURM_JOB_ID/ -z 128
else
    srun python 3_code/tf_vae_tfrecords.py -c triton --data_path /tmp/$SLURM_JOB_ID/ -z 128 --batch_normalization
fi
