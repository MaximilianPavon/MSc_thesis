...
class MyCallbackDecoder_NDVI(tf.keras.callbacks.Callback):
    def __init__(self, decoder, log_dir, num_examples_to_generate=16, log_freq=10):
        super(MyCallbackDecoder_NDVI, self).__init__()
        self.decoder = decoder
        self.log_dir = log_dir
        self.num_examples_to_generate = num_examples_to_generate
        self.latent_dim = self.decoder.input[0].shape.dims[0].value
...
class MyCallbackCompOrigDecoded_NDVI(tf.keras.callbacks.Callback):
    def __init__(self, log_dir, X, num_examples=10, log_freq=10):
        super(MyCallbackCompOrigDecoded_NDVI, self).__init__()
        self.log_dir = log_dir
        self.X = X
        self.num_examples = num_examples
        self.log_freq = log_freq
        ...
        # select num_examples from X and store arrays to keep them constant over all epochs
        self.orig_images = self.X[np.random.choice(self.X.shape[0], self.num_examples, replace=False), :, :, :]
