...
def _parse_function(example_proto):
    feature_description = {
        ...
        'full_crop_loss_label': tf.io.FixedLenFeature([], tf.float32),
        'partial_crop_loss_label': tf.io.FixedLenFeature([], tf.float32),
    ...
    parsed_features = tf.io.parse_single_example(serialized=example_proto, features=feature_description)

    # Decode saved image string into an array
    image = tf.io.decode_raw(parsed_features['image'], tf.float32)  # tensor is still flattened
